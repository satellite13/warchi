...
- Пользовательские формы узлов с отдельным редактором контура
...
- Встроенный раздел документации (`/docs`) с гайдами, хоткеями, FAQ и changelog
- Раздел `Wiki` (`/wiki`) для документации, привязанной к сущностям
...
| `VITE_MODEL_LIVE_SYNC_MODE` | `hybrid` | Режим live sync модели: `ws`, `poll` или `hybrid` (WebSocket + fallback polling) |
| `VITE_MODEL_LIVE_POLL_MS` | `15000` | Интервал опроса (мс) для режима `poll` и fallback в `hybrid` |
...
- Раздел документации в приложении: `/docs` (обзор, модели, нотации, диаграммы, типы, формы, горячие клавиши, FAQ, история изменений)
- Раздел `Wiki`: `/wiki` (страницы, связанные с моделями, нотациями, типами и формами)
- Исходники контента раздела документации: `src/features/docs/content/*.md`
- Технические планы и заметки по реализации: `docs/*.md`
