...
На момент Task 13 benchmark не запускался: численные результаты для этой ветки не
записывались. Сетевые сценарии должны проверяться изолированной e2e-фикстурой, а не UUID
эталонной модели, чтобы тест не зависел от конкретного окружения.
