...
The fixed default versions are arepos-server `0.5.2`, warchi `0.8.15`, and warchi-site
`0.2.3`. The scripts never deploy Papirus source; the warchi release consumes
...
- each repository clean, with `warchi` and `arepos-server` checked out on `master` and
  ...
  (`v0.8.15`, `v0.5.2`, or `v0.2.3`); these release branches are pinned rather than inferred from
