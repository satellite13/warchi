...
The fixed default versions are arepos-server `0.5.2`, warchi `0.8.12`, and warchi-site
...
- each repository clean, with `warchi` and `arepos-server` checked out on `master` and
  ...
  (`v0.8.12`, `v0.5.2`, or `v0.2.1`); these release branches are pinned rather than inferred from
