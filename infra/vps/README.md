...
The fixed default versions are arepos-server `0.5.5`, warchi `0.8.18`, and warchi-site
...
- each repository clean, with `warchi` and `arepos-server` checked out on `master` and
  ...
  (`v0.8.18`, `v0.5.5`, or `v0.2.4`); these release branches are pinned rather than inferred from
...
1. `arepos-server`: atomic Helm upgrade, rollout, API `0.5.5`, Liquibase migration `043`;
