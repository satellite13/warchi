...
const modelsTs = fs.readFileSync('src/i18n/locales/models.ts', 'utf8')
const wrapped = modelsTs.replace('export const modelsMessages', 'globalThis.modelsMessages')
vm.runInNewContext(wrapped, { globalThis: globalThis })
const en = globalThis.modelsMessages.en.models
...
const fr = {
  ...
  diagramConflictMessage:
    'Un diagramme avec ce nom et cette version existe déjà dans le modèle.',
  ...
  validationLinkPropRequired:
    'Le lien « {link} » n’a pas la propriété obligatoire « {prop} ».',
...
const extra = Object.keys(fr).filter((k) => !keys.includes(k))
if (extra.length > 0) {
  console.error('Extra FR keys:', extra)
  process.exit(1)
}
...
let out = `/** Generated by scripts/build-models-fr.mjs — do not edit by hand; run \`node scripts/build-models-fr.mjs\`. */\n`
out += `export const modelsMessagesFr = {\n  models: {\n`
for (const k of keys) {
  out += `    ${k}: '${esc(fr[k])}',\n`
}
out += `  },\n}\n`

fs.writeFileSync('src/i18n/locales/models.fr.generated.ts', out)
console.log('Wrote src/i18n/locales/models.fr.generated.ts', keys.length, 'keys')
