...
import type { AccessShareRequest, AccessShareResponse, ShareResourceType } from '../types/api'
...
export function useAccessShares() {
  ...
  const loadShares = async (resourceType: ShareResourceType, resourceId: string): Promise<void> => {
    ...
    try {
      ...
      errorMessage.value = error instanceof Error ? error.message : t('share.loadSharesError')
      shares.value = []
    } finally {
  ...
  const grantShare = async (payload: AccessShareRequest): Promise<boolean> => {
    ...
    try {
      ...
      errorMessage.value = error instanceof Error ? error.message : t('share.grantShareError')
      return false
    } finally {
  ...
  const revokeShare = async (
    ...
    try {
      ...
      errorMessage.value = error instanceof Error ? error.message : t('share.revokeShareError')
      return false
    } finally {
      isSubmitting.value = false
    }
