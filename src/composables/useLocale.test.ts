...
vi.mock('../i18n', () => ({
  ...
  isSupportedLocale: (value: string | null) => value === 'ru' || value === 'en' || value === 'fr',
}))
...
describe('useLocale', () => {
  ...
  it('returns correct supportedLocales array', () => {
    ...
    expect(supportedLocales).toEqual(['ru', 'en', 'fr'])
  })
  ...
  describe('setLocale', () => {
    ...
    it('sets locale to fr', async () => {
      const { currentLocale, setLocale } = useLocale()
      setLocale('fr')
      await nextTick()

      expect(currentLocale.value).toBe('fr')
      expect(localStorage.getItem('warchi.locale')).toBe('fr')
  ...
  describe('localStorage persistence', () => {
    ...
    it('updates localStorage on subsequent sets', async () => {
      const { setLocale } = useLocale()
      setLocale('en')
      await nextTick()
      expect(localStorage.getItem('warchi.locale')).toBe('en')
