  ...
  for (const row of matrix.rows) {
    for (const column of matrix.columns) {
      ...
      for (const relationId of relationIds) {
        const relationCount = relationId
          ? (cell.relationCounts[relationId] ?? cell.total)
          : cell.total
...
function buildWideCsv(matrix: RelationMatrixResult): string {
  const header = ['row\\col', ...matrix.columns.map(col => col.name)]
    .map(escapeCsv)
    .join(CSV_DELIMITER)
...
export function downloadRelationMatrixCsv(params: {
  ...
  const csv =
    params.format === 'long'
      ? buildLongCsv(params.matrix, {
          notationId: params.notationId,
          notationName: params.notationName,
        })
