export function canvasModelNodeIds(instances: {
  nodes: Array<{ modelNodeId?: string }>
}): string[] {
  return instances.nodes.map(node => node.modelNodeId).filter((id): id is string => Boolean(id))
}
