...
function normalizeStylePreset(
  data: StylePresetJson
): { name: string; label: string; style: DiagramStyle } | null {
...
export function subscribeStylePresetsChanges(callback: () => void): () => void {
  ...
  const handleStorageChange = (event: StorageEvent) => {
    if (event.key === USER_COMPONENT_PRESETS_KEY || event.key === USER_RELATION_PRESETS_KEY) {
      callback()
    }
  }
