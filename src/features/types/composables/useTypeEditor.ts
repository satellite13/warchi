...
export function useTypeEditor() {
    ...
    try {
      if (item._isNew) {
        if (item.kind === 'node') {
          ...
          if (!result.success) {
            failSave(formatTypeOperationError('save', result.error.status, result.error.message))
            return false
          }
  ...
  async function deleteType(item: TypeItem): Promise<boolean> {
    ...
    try {
      ...
      if (!result.success) {
        failSave(formatTypeOperationError('delete', result.error.status, result.error.message))
        return false
      }
      removeLocal(item)
      completeSave()
