/** Generated by scripts/build-models-fr.mjs — do not edit by hand; run `node scripts/build-models-fr.mjs`. */
export const modelsMessagesFr = {
  models: {
    entityName: 'Modèle',
    entityNamePlural: 'modèles',
    entityLabelAccusative: 'modèle',
    conflictMessage: 'Un modèle avec ce nom et cette version existe déjà',
    notFoundMessage: 'Modèle introuvable',
    ownerNotFoundMessage: 'Propriétaire introuvable',
    renameFailed: 'Échec du renommage du modèle',
    enterName: 'Nom du modèle',
    searchPlaceholder: 'Rechercher par nom...',
    createTitle: 'Créer un modèle',
    createDescription: 'Nouveau modèle d\'architecture',
    emptySourceVersion: 'Modèle vide',
    notFoundTitle: 'Aucun modèle',
    notFoundDescription: 'Modifiez la recherche',
    namePlaceholder: 'Nom du modèle',
    deleteTitle: 'Supprimer le modèle',
    renameTitle: 'Renommer le modèle',
    accessTitle: 'Accès au modèle',
    versionTreeTitle: 'Arbre des versions : {name}',
    versionTreeEmpty: 'Aucune filiation de versions.',
    versionTreeMultipleRoots: 'Plusieurs branches indépendantes (créées depuis zéro ou source hors liste).',
    documentation: 'Page du Wiki',
    diagramDocumentation: 'Page diagramme dans le Wiki',
    oefImportTitle: 'Importer Open Exchange (XML)',
    oefImportTargetNotation: 'Notation cible',
    oefImportSourceFile: 'Fichier d’import (.xml)',
    oefImportSelectedFile: 'Fichier sélectionné : {name}',
    oefImportElementMappings: 'Mappage des types d’éléments',
    oefImportRelationshipMappings: 'Mappage des types de relations',
    oefImportIssuesTitle: 'Problèmes des données d’entrée',
    oefImportWarningsHint: 'L’import sera effectué avec des avertissements.',
    oefImportRun: 'Importer',
    oefImportStatNodes: 'Nœuds : {count}',
    oefImportStatLinks: 'Liens : {count}',
    oefImportStatDiagrams: 'Diagrammes : {count}',
    oefImportReadError: 'Impossible de lire ou d’analyser le fichier XML d’import.',
    oefImportNoChanges: 'Aucun changement à enregistrer dans cet import.',
    oefImportFailed: 'Échec de l’import : {message}',
    oefImportCompletedWithWarnings: 'Import terminé avec avertissements : {count}',
    oefImportStepAnalyze: 'Analyse',
    oefImportStepMapping: 'Mappage',
    oefImportStepPreview: 'Aperçu',
    oefImportShowOnlyUnmapped: 'Afficher seulement les types non mappés',
    oefImportMappedProgress: 'Mappé : {mapped} / {total}',
    oefImportBulkSelect: 'Choisissez un mapping pour l’application en masse',
    oefImportApplyToVisible: 'Appliquer aux éléments visibles',
    oefImportPrevStep: 'Précédent',
    oefImportNextStep: 'Suivant',
    oefImportIssuesBlocking: 'Le fichier contient des erreurs. Corrigez-les avant de poursuivre.',
    oefImportPreviewMapped: 'Prêt à importer : nœuds {nodes}, liens {links}',
    oefImportReportTitle: 'Rapport d’import',
    oefImportReportSummary: 'Import terminé. Entités créées :',
    oefImportReportDiagramNodeInstances: 'Instances de nœuds sur les diagrammes : {count}',
    oefImportReportDiagramEdgeInstances: 'Instances de liens sur les diagrammes : {count}',
    oefImportReportWarningsByReason: 'Avertissements par cause :',
    oefImportReportMissingRequiredTitle:
      'Propriétés obligatoires sans valeur après import : {count}',
    oefImportReportMissingRequiredNodeType: 'Propriétés des types de nœuds : {count}',
    oefImportReportMissingRequiredComponent: 'Propriétés des composants : {count}',
    oefImportReportMissingRequiredRelation: 'Propriétés des relations : {count}',
    oefImportWarningNodeTypeNotMapped: 'Type d’élément non mappé',
    oefImportWarningLinkTypeNotMapped: 'Type de relation non mappé',
    oefImportWarningLinkMissingNode: 'Lien ignoré : nœud source/cible introuvable',
    oefImportWarningDiagramNodeMissingModelNode:
      'Instance de nœud du diagramme ignorée : nœud du modèle introuvable',
    oefImportWarningDiagramConnectionMissingModelLink:
      'Connexion du diagramme ignorée : lien du modèle introuvable',
    oefImportWarningDiagramConnectionMissingNodeInstance:
      'Connexion du diagramme ignorée : instance source/cible introuvable',
    showDiagramSettings: 'Afficher les paramètres du diagramme',
    hideDiagramSettings: 'Masquer les paramètres',
    enableSelectionSync: 'Activer la synchro de sélection',
    disableSelectionSync: 'Désactiver la synchro de sélection',
    addRootFolder: 'Ajouter un dossier racine',
    addRootNode: 'Ajouter un nœud racine',
    noNodes: 'Aucun nœud',
    createFolderOrNodeHint: 'Créez un dossier ou un nœud depuis l’en-tête',
    addChildFolder: 'Ajouter un sous-dossier',
    addChildNode: 'Ajouter un nœud enfant',
    renameFolder: 'Renommer le dossier',
    renameDiagram: 'Renommer le diagramme',
    openDiagramDoubleClick: 'Ouvrir le diagramme (double-clic)',
    diagramOpened: 'Ouvert',
    diagramLockHeldBy: 'Diagramme verrouillé par {name}',
    diagramLockRetryEdit: 'Réessayer d\'éditer',
    diagramLockReload: 'Recharger depuis le serveur',
    diagramLockBadgeYou: 'Vous détenez le verrou d’édition',
    diagramLockBadgeOther: 'Édité par {name}',
    diagramLockForceRevoked: 'Verrou libéré par un administrateur. L’édition du diagramme n’est plus disponible.',
    resizeLeftPanelWidth: 'Glisser pour redimensionner le panneau gauche',
    resizeRightPanelWidth: 'Glisser pour redimensionner le panneau droit',
    showLeftPanel: 'Afficher le panneau gauche',
    hideLeftPanel: 'Masquer le panneau gauche',
    showRightPanel: 'Afficher le panneau droit',
    hideRightPanel: 'Masquer le panneau droit',
    createFolderTitle: 'Créer un dossier',
    createNodeTitle: 'Créer un nœud',
    newFolderPlaceholder: 'Nouveau dossier',
    newNodePlaceholder: 'Nouveau nœud',
    nodeTypeLabel: 'Type de nœud',
    nodeTypeProperties: 'Propriétés du type de nœud',
    notationComponentProperties: 'Propriétés du composant de notation',
    propertySourceBadgeType: 'Type de nœud',
    propertySourceBadgeComponent: 'Composant',
    propertyDiagramLabelTokenHint: 'Modèle d\'étiquette sur la figure : # et clé = propriété du type de nœud ; $ et clé = propriété du composant de notation. Le nom du nœud utilise un espace réservé séparé : dollar et le mot name.',
    selectType: 'Choisir un type',
    typeSearchPlaceholder: 'Rechercher un type...',
    directoryTypeHint: 'Le type Directory sera utilisé.',
    noteTextLabel: 'Texte',
    noteTextPlaceholder: 'Saisir le texte de la note...',
    createDiagramTitle: 'Créer un diagramme',
    newDiagramPlaceholder: 'Nouveau diagramme',
    notationLabel: 'Notation',
    diagramConflictMessage: 'Un diagramme avec ce nom et cette version existe déjà dans le modèle.',
    diagramVersion: 'Version du diagramme',
    viewOnly: 'Lecture seule',
    docLinkRegisterFailed: 'Impossible de lier le document au modèle : {message}',
    createBaseline: 'Créer une version de base',
    baselineCreateError: 'Échec de la création de la ligne de base du diagramme.',
    existingLinksFoundTitle: 'Liens existants trouvés',
    useExistingLink: 'Utiliser l’existant',
    reuseLinkTypeLabel: 'Type de lien',
    reuseLinkCustomPropertiesLabel: 'Propriétés personnalisées de l’instance',
    reuseLinkNoCustomProperties: 'Aucune propriété personnalisée',
    createNewLink: 'Créer un nouveau lien',
    unsavedChangesTitle: 'Modifications non enregistrées',
    saveBeforeCloseDiagram: 'Des modifications ne sont pas enregistrées. Les enregistrer avant de fermer le diagramme ?',
    saveBeforeSwitchDiagram: 'Des modifications ne sont pas enregistrées. Les enregistrer avant de changer de diagramme ?',
    dontSave: 'Ne pas enregistrer',
    saveAndClose: 'Enregistrer et fermer',
    saveAndSwitch: 'Enregistrer et changer',
    deleteNodeTitle: 'Supprimer le nœud',
    deleteNodeFromDiagramSingle: 'Supprimer le nœud {name} du diagramme actuel ?',
    deleteNodeFromDiagramMultiple: 'Supprimer {count} nœud(s) du diagramme actuel ?',
    deleteNodeFromModelSingle: 'Supprimer le nœud {name} du modèle ?',
    deleteNodeFromModelMultiple: 'Supprimer {count} nœud(s) du modèle ?',
    deleteDiagramTitle: 'Supprimer le diagramme',
    deleteDiagramConfirm: 'Supprimer le diagramme {name} ?',
    deleteLinkTitle: 'Supprimer le lien',
    deleteLinkQuestion: 'Que faire avec le lien sélectionné ?',
    removeLinkFromDiagram: 'Retirer du diagramme',
    removeLinkFromModel: 'Retirer du modèle',
    leaveUnsavedText: 'Vous avez des modifications non enregistrées. Si vous quittez la page, elles seront perdues.',
    batchSaveConflictTitle: 'Conflit à l’enregistrement',
    batchSaveConflictIntro: '{count} objet(s) ont changé sur le serveur depuis le début de votre édition. L’enregistrement a été annulé — choisissez une action.',
    batchSaveConflictNotOnlyListedHint: 'Seuls les objets en décalage de version sont affichés. Après rechargement, vérifiez l’arbre et les diagrammes.',
    batchSaveConflictCrossDeletedLinksTitle: 'Arêtes pointant vers des liens supprimés côté serveur',
    batchSaveConflictCrossDeletedLinksLoading: 'Vérification des liens…',
    batchSaveConflictChoicesAria: 'Options de résolution du conflit',
    batchSaveConflictChoiceReloadTitle: 'Recharger depuis le serveur',
    batchSaveConflictChoiceReloadDesc: 'Charger les données actuelles du serveur en conservant votre canevas. Puis enregistrez à nouveau.',
    batchSaveConflictChoiceOverwriteTitle: 'Écraser le serveur',
    batchSaveConflictChoiceOverwriteDesc: 'Forcer l’enregistrement de vos données. Les modifications des autres seront perdues.',
    batchSaveConflictReload: 'Recharger depuis le serveur',
    batchSaveConflictOverwrite: 'Écraser',
    batchSaveConflictCompareToggle: 'Comparer',
    batchSaveConflictCompareTimestampOnly: 'Aucune différence de champs — seul l’horodatage diffère. Le rechargement synchronisera.',
    batchSaveConflictCompareTimestampSinceDiagramOpen: 'Le contenu correspond au serveur ; seul l’horodatage diffère.',
    batchSaveConflictDiagramCanvasPending: 'Canevas (instances sur le diagramme)',
    batchSaveConflictDiagramCanvasCounts: '{nodes} nœuds, {edges} arêtes sur le canevas',
    batchSaveConflictDiagramDocFile: 'Documentation (fichier lié)',
    batchSaveConflictDiagramNodeOnlyLocal: 'Sur le canevas : « {name} » — chez vous seulement',
    batchSaveConflictDiagramNodeOnlyServer: 'Sur le canevas : « {name} » — serveur seulement',
    batchSaveConflictDiagramNodeGeom: 'Sur le canevas : « {name} » — position et taille',
    batchSaveConflictDiagramNodeComponentProps: 'Sur le canevas : « {name} » — propriétés des composants',
    batchSaveConflictDiagramNodeOtherAttrs: 'Sur le canevas : « {name} » — autres attributs d’instance',
    batchSaveConflictDiagramEdgeOnlyLocal: 'Sur le canevas : {link} — chez vous seulement',
    batchSaveConflictDiagramEdgeOnlyServer: 'Sur le canevas : {link} — serveur seulement',
    batchSaveConflictDiagramEdgeRelationProps: 'Sur le canevas : {link} — propriétés de relation',
    batchSaveConflictDiagramEdgeOtherAttrs: 'Sur le canevas : {link} — autres attributs d’instance',
    batchSaveConflictDiagramCanvasChanged: 'Structure ou données du canevas modifiées (résumé)',
    batchSaveConflictDiagramOnCanvas: 'sur le canevas',
    batchSaveConflictDiagramEdgeBetweenNodes: 'sur le canevas entre « {from} » et « {to} »',
    batchSaveConflictDiagramEdgeLinkId: 'lien {id}',
    batchSaveConflictUnknownNode: 'nœud absent de l’arbre du modèle ({id})',
    batchSaveConflictUnknownLink: 'lien absent de la liste de l’éditeur ({id})',
    batchSaveConflictDiagramEdgeLinkMissingHint: 'absent du modèle chargé dans l’éditeur (souvent après suppression du lien par un autre utilisateur)',
    batchSaveConflictDiagramEdgeOrphanTitle: 'Arête sans lien modèle dans l’éditeur : {link}',
    batchSaveConflictDiagramOrphanLocalCol: 'l’instance existe dans les données du diagramme ; le lien manque dans la liste chargée',
    batchSaveConflictDiagramOrphanServerAlsoInJson: 'le JSON serveur contient encore cette arête (le lien modèle est déjà supprimé)',
    batchSaveConflictDiagramOrphanServerNotInJson: 'le JSON serveur ne contient plus cette arête',
    batchSaveConflictCompareLocal: 'Dans l’éditeur (charge utile batch-save)',
    batchSaveConflictCompareServer: 'Sur le serveur (GET par id)',
    batchSaveConflictCompareLoading: 'Chargement de la version serveur…',
    batchSaveConflictCompareError: 'Échec du chargement',
    batchSaveConflictLocalMissing: 'Absent de l’état local de l’éditeur',
    batchSaveConflictFieldColField: 'Champ',
    batchSaveConflictFieldColLocal: 'Chez vous',
    batchSaveConflictFieldColServer: 'Sur le serveur',
    batchSaveConflictRepeatHint: 'Le prochain enregistrement reverra le serveur.',
    batchSaveConflictKindNode: 'Nœud',
    batchSaveConflictKindLink: 'Lien',
    batchSaveConflictKindDiagram: 'Diagramme',
    batchSaveConflictUnnamedNode: '(sans nom)',
    batchSaveConflictEntityMissing: '{kind} (pas dans le modèle ouvert)',
    batchSaveConflictLinkLine: '{from} → {to}',
    batchSaveConflictLinkWithType: '{type} : {from} → {to}',
    batchSaveConflictDiagramLine: '« {name} », version {version}',
    batchSaveConflictYourBaseTime: 'Vos modifications partaient de l’état du {time}',
    batchSaveConflictServerTime: 'dernière modification sur le serveur : {time}',
    batchSaveConflictNodeTypeLabel: 'Type : {name}',
    batchSaveConflictNodeTypeUnknown: 'Type : inconnu',
    batchSaveConflictNodeRootParent: 'Parent : racine de l’arbre',
    batchSaveConflictNodeParentLabel: 'Parent : « {name} »',
    batchSaveConflictNodeParentMissing: 'Parent : introuvable dans le modèle',
    stay: 'Rester',
    leave: 'Quitter',
    diagramJsonTitle: 'JSON du diagramme',
    copy: 'Copier',
    propertiesTab: 'Propriétés',
    traceabilityTab: 'Traçabilité',
    relationMatrixOpen: 'Matrice des relations',
    relationMatrixTitle: 'Matrice des relations des composants',
    relationMatrixNotationLabel: 'Notation',
    relationMatrixWithoutNotation: 'Sans notation (types)',
    relationMatrixRowsLabel: 'Axe vertical',
    relationMatrixColumnsLabel: 'Axe horizontal',
    relationMatrixRelationsLabel: 'Relations',
    relationMatrixMappedOnly: 'Seulement les éléments mappés',
    relationMatrixHeatmapToggle: 'Heatmap',
    relationMatrixHideEmptyAxes: 'Masquer les lignes et colonnes vides',
    relationMatrixSelectAll: 'Tout sélectionner',
    relationMatrixClearSelection: 'Effacer la sélection',
    relationMatrixExportCsvLong: 'CSV long',
    relationMatrixExportCsvWide: 'CSV wide',
    relationMatrixExportPng: 'PNG',
    relationMatrixNoLinks: 'Aucun lien',
    relationMatrixLinksCount: 'Liens',
    relationMatrixAxesLabel: 'Vertical \\ Horizontal',
    relationMatrixTotalLabel: 'Total',
    relationMatrixUnmapped: 'Non mappé',
    relationMatrixVerticalInfoTitle: 'Composant vertical',
    relationMatrixHorizontalInfoTitle: 'Composant horizontal',
    relationMatrixNodeType: 'Type de nœud',
    relationMatrixNodesCount: 'Nœuds dans l’échantillon',
    relationMatrixCustomProperties: 'Propriétés personnalisées',
    relationMatrixNoCustomProperties: 'Aucune donnée de propriété personnalisée',
    relationMatrixLinksListTitle: 'Liens',
    relationMatrixLinkCustomPropertiesTitle: 'Propriétés du lien',
    relationMatrixLinkDiagramsTitle: 'Utilisé dans les diagrammes',
    relationMatrixNoDiagramsForLink: 'Le lien n’est placé sur aucun diagramme',
    relationMatrixValueEmpty: '(vide)',
    relationMatrixDetailsEmpty: 'Sélectionnez une cellule de la matrice pour inspecter les cartes de relations.',
    relationMatrixUnknownRelation: 'Type de relation inconnu',
    relationMatrixNotationFallbackPrefix: 'Notation',
    figureStyleTab: 'Style de figure',
    traceabilityNoNode: 'Sélectionnez un nœud pour la traçabilité',
    traceabilityNoNodeHint: 'Un nœud sur le diagramme ou dans l’arbre du modèle',
    traceabilityDirectionDown: 'Descendre',
    traceabilityDirectionUp: 'Remonter',
    traceabilityAllLinkTypes: 'Tous les types de liens',
    traceabilityUnknownLinkType: 'Type inconnu',
    traceabilityDiagramsTitle: 'Utilisé dans les diagrammes',
    traceabilityNoDiagrams: 'Ce nœud n’est placé sur aucun diagramme',
    traceabilityTreeTitle: 'Arbre de traçabilité',
    traceabilityDragHint: 'Glissez le lien sur le diagramme',
    traceabilityDragDisabledNoActiveDiagram: 'Ouvrez un diagramme pour glisser ce lien',
    traceabilityDragDisabledReadOnly: 'Le diagramme actif est en lecture seule',
    traceabilityDragDisabledAlreadyOnDiagram: 'Ce lien est déjà sur le diagramme',
    traceabilityDragDisabledMissingEndpointInstances: 'Les deux extrémités du lien doivent être sur le diagramme pour le glisser',
    traceabilityDragDisabledMissingRelation: 'Aucune relation pour ce type de lien dans la notation active',
    traceabilityDragDisabledConnectNotAllowed: 'Les règles de notation interdisent de relier ces nœuds',
    defaultLinkType: 'Type de ligne par défaut',
    autoLinkInGroups: 'Liens automatiques dans les groupes',
    compareWithVersion: 'Comparer avec une version',
    compareTitle: 'Comparaison des versions du modèle',
    compareSelectVersion: 'Choisir la version à comparer',
    compareLoadingVersions: 'Chargement des versions…',
    compareLoadingTarget: 'Chargement des données de version…',
    compareNoVersions: 'Aucune autre version de ce modèle',
    compareDiffNodes: 'Nœuds',
    compareDiffLinks: 'Liens',
    compareDiffDiagrams: 'Diagrammes',
    compareDiffAdded: 'Ajouté',
    compareDiffRemoved: 'Supprimé',
    compareDiffModified: 'Modifié',
    compareClose: 'Fermer',
    compareNoChanges: 'Aucun changement',
    compareVisualTitle: 'Comparaison visuelle des versions',
    compareDiagramVersions: 'Comparer les versions du diagramme',
    compareDiagramVersionsTitle: 'Comparaison des versions du diagramme',
    compareVersionLeft: 'Version à gauche',
    compareVersionRight: 'Version à droite',
    compareBaseLabel: 'Base',
    compareChangesLabel: 'Modifications',
    compareDiagramName: 'Diagramme',
    compareNoDiagram: 'Aucun diagramme',
    compareVisualOpen: 'Comparaison visuelle',
    compareSwapVersions: 'Inverser les côtés',
    compareToggleBase: 'Changer la base',
    compareSelectedElement: 'Élément sélectionné',
    comparePropName: 'Propriété',
    compareLinkRoute: 'Tracé du lien',
    comparePropWas: 'Était (gauche)',
    comparePropWasRight: 'Était (droite)',
    comparePropBecame: 'Devenu (droite)',
    comparePropBecameLeft: 'Devenu (gauche)',
    relationRulesLoading: 'Chargement des règles de relation pour la notation actuelle…',
    relationRulesLoadingConnectBlocked: 'Les règles de relation se chargent encore. Réessayez dans quelques secondes.',
    notationLoading: 'Chargement de la notation…',
    notationUnavailable: 'Notation indisponible',
    notationAccessDenied: 'Pas d’accès à la notation',
    notationLoadFailed: 'Échec du chargement de la notation',
    notationMetaUnavailable: 'Métadonnées de notation indisponibles. Fonctionnalités du diagramme limitées.',
    notationRelationRulesLoadFailed: 'Échec du chargement des règles de relation pour la notation active.',
    notationRelationRulesRefreshFailed: 'Échec de l’actualisation des relations et règles pour le diagramme.',
    directoryTypeNotFound: 'Type Directory introuvable. Impossible de créer le dossier.',
    noAvailableNodeTypes: 'Aucun type de nœud disponible hormis Directory.',
    diagramSwitchFailed: 'Impossible d’ouvrir le diagramme sélectionné après actualisation des données.',
    noDiagramSelected: 'Aucun diagramme sélectionné',
    noteName: 'Note',
    unknownLinkType: 'Type inconnu',
    batchSaveInvalidResponse: 'Réponse serveur incorrecte pour l’enregistrement groupé.',
    batchSaveVersionConflict: 'Conflit de version à l’enregistrement (données modifiées sur le serveur).',
    validationNodeTypePropRequired: 'Le nœud « {node} » n’a pas la propriété de type obligatoire « {prop} ».',
    validationNodeComponentPropRequired: 'Le nœud « {node} » n’a pas la propriété de composant obligatoire « {prop} » sur le diagramme « {diagram} ».',
    validationLinkPropRequired: 'Le lien « {link} » n’a pas la propriété obligatoire « {prop} ».',
    noMatchingComponent: 'Aucun composant de notation correspondant à ce type de nœud.',
    cannotCreateNodeWithoutDirectory: 'Impossible de créer un nœud sans répertoire de diagramme actif.',
    directoryTypeRequiredForAutoPath: 'Le type de nœud Directory est requis pour la création automatique du chemin.',
    newNoteText: 'Nouvelle note',
    noComponentsForLink: 'Sélectionnez les composants de notation pour les deux nœuds avant de créer un lien.',
    noAllowedRelationRules: 'Aucune relation autorisée pour cette paire de composants selon la notation.',
    noAvailableRelations: 'Aucune relation disponible pour cette paire de composants selon la notation.',
    figureComponentNotFound: 'Composant de notation introuvable pour la figure sélectionnée.',
    edgeRelationNotFound: 'Relation de notation introuvable pour l’arête sélectionnée.',
  },
}
