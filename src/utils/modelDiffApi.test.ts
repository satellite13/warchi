...
function makeLocalDiagram(overrides: Partial<DiagramResponse> = {}): DiagramResponse {
  return {
    id: 'ld1',
    name: 'Diag',
    version: '1.0.0',
...
describe('fetchModelDiff', () => {
  ...
  it('returns normalized diff on successful API call', async () => {
    ...
    expect(mockApiGet).toHaveBeenCalledWith('/models/base-id/diff/target-id')
    expect(diff.nodes).toHaveLength(2)
    expect(diff.links).toHaveLength(1)
    expect(diff.diagrams).toHaveLength(1)
  ...
  it('throws when API fails and no localData provided', async () => {
    ...
    await expect(fetchModelDiff('base-id', 'target-id')).rejects.toThrow('Internal Server Error')
