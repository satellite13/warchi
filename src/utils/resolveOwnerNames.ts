...
export async function resolveOwnerDisplayNames(
  ...
  if (currentUser?.id && hasResolvableDisplayName(currentUser)) {
    result.set(normalizeOwnerId(currentUser.id), getUserDisplayName(currentUser, fallback))
  }
  ...
  const batchResult = await apiPost<PaginatedResponse<UserInfo>>('/users/public/batch', {
    ids: toLoad,
  })
  if (batchResult.success) {
    for (const user of paginatedContent(batchResult.data)) {
      result.set(normalizeOwnerId(user.id), getUserDisplayName(user, user.email ?? fallback))
    }
